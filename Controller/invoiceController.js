...
const createInvoice = async (req, res) => {
  ...
  // Basic validations
  if (!invoice_number || !customer_name || !customer_phone || !invoice_date || !payment_method || !total_amount || !items || !Array.isArray(items) || items.length === 0) {
    return res.status(400).json({ error: 'All invoice fields and at least one item are required.' });
  }
  ...
  try {
    ...
    const dupCheck = await client.query('SELECT id FROM invoices WHERE invoice_number = $1', [invoice_number]);
    ...
    for (const item of items) {
      ...
      if (!item_name || !quantity || !unit_price) {
        await client.query('ROLLBACK');
        return res.status(400).json({ error: 'Invalid item entries. Name, quantity, and unit price are required.' });
      }
    ...
    return res.status(500).json({ error: 'Server error during invoice creation.' });
  }
};
...
const getInvoiceById = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: 'Server error fetching invoice details.' });
  }
};
